...
class Agregar extends Component
{
    ...
    public function terminarCompetencia($id)
    {
        ...
        $competencia->save();
        $this->otorgarPuntos($id);
        $this->emit('recarga');
        $this->emit('msjAccion', [true, 'Finalizo la competencia correctamente.']);
        $this->open = false;
    }

    private function otorgarPuntos($id)
    {
        $c_competidor = CompetenciaCompetidor::where('id_competencia', $id)->orderBy('calificacion', 'desc')->get();
        for ($i = 0; $i < count($c_competidor); $i++) { 
            if($c_competidor[$i]->calificacion != null || $c_competidor->calificacion != ""){
                $user = User::find($c_competidor[$i]->id_competidor);
                if ($i == 0) {
                    $user->clasificacion += 3;
                }
                if ($i == 1) {
                    $user->clasificacion += 2;
                }
                if ($i == 2) {
                    $user->clasificacion += 1;
                }
                $user->save();
            }
        }
